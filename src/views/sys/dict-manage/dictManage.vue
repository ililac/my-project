    ...
    <Card>
      <Row type="flex" justify="space-between" class="code-row-bg">
        <Col v-if="expand" span="5">
          ...
          <Alert show-icon>
            当前选择： <span class="select-title" @click="parentNode" style="cursor: pointer;">{{editTitle}}</span>
            <span class="select-title">{{childName}}</span>
          ...
          <div class="tree-bar">
            <Tree :load-data="loadData" :data="treeData" @on-select-change="selectTree"></Tree>
          </div>
  ...
  methods: {
      ...
      getAllDictList().then(res => {
        ...
        if (res.success === true) {
					...
					for(var i = 0;i<res.result.length;i++){
						if(res.result[i].haveChild){
							res.result[i].children = [];
							res.result[i].loading = false;
						}
					}
    ...
    edit(v) {
      ...
      this.modalVisible = true;
		...
		child(v){
			this.loading = true;
			this.childName = "--" + v.title;
			this.searchForm.dictId = v.id;
			getAllDictDataList(this.searchForm).then(res => {
			  this.loading = false;
			  if (res.success === true) {
			    this.data = res.result.content;
