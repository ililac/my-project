		...
		<Modal :title="modalTitle" v-model="modalVisible" :mask-closable='false' id="allWin"  :width="1000" class="formModel" @on-visible-change="modalVisiblechange">
			<Form ref="dictForm" :model="dictForm" :label-width="120" :rules="dictFormValidate">
				...
				<div class="ul">
					...
					<FormItem class="lef" v-show="dictForm.istopay != 2">
						<span style="position: absolute;left: -100px;top: 0;"><b style="color: #ed4014;font-size: 14px;line-height: 33px;">*</b>合同金额<b style="color: #ed4014;">（万元）</b></span>
						<Input v-model="dictForm.money" v-on:input="nuMinput(dictForm.money)"/>
					</FormItem>
				</div>	
				<div class="ul">
				...
				<Col :span="24">
					<FormItem label="相关附件">
						<Upload 
							...
							class="upload"
							>
							<p id="attachmentname" class="upload" @click="getUpId('attachmentname')"><Icon type="ios-cloud-upload-outline" size="20"/>上传文件</p>
						</Upload>
...
// 					this.dictForm[res.result[0].tag.split("-")[1]] = res.result[0].name;
			...
			handleSuccess2(res, file) {
				if(res.result == "success"){
					...
					this.fileDownUrl = '/xboot/draft/download?fileName='+res.result.name+'&url='+res.result.url+'&accessToken='+this.getStore("accessToken");
			...
			beforeUpload(file) {
				console.log(document.getElementById("contentname"));
		...
		　　 	handler(newValue, oldValue) {
					eachOther().then(res => {
						...
						if(this.dictForm.attachmentname){
							this.fileDownUrl2 = "/xboot/draft/download?id="+this.dictForm.id+"&tag=2&accessToken="+this.getStore("accessToken");
						}else{
							this.fileDownUrl2 = "";
						}
