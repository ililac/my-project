  ...
  <div>
    ...
    >
      <Form ref="dictForm" :model="dictForm" :label-width="120" :rules="dictFormValidate">
        <div v-if="dictForm.modalSource == 1">
          ...
          <div class="ul">
            <FormItem label="变更内容" class="asterisk">
              <Input type="textarea" v-model="dictForm.changeContent" :maxlength="1000" :rows="2" />
            </FormItem>
          </div>
        ...
        <div class="ul three">
          <FormItem label="合同正文" v-if="!sourceShow" class="lef" prop="contentname">
            ...
            <p v-show="fileDownUrl">
              <a v-bind:href="fileDownUrl">{{dictForm.contentname}}</a>
            </p>
          </FormItem>
          <FormItem label="合同正文" v-else="sourceShow" class="lef" prop="contentname">
            ...
            <p
              v-show="fileDownUrl"
              @click="fileDowm(dictForm.contentname,dictForm.contracturl)"
              style="color: #2B85E4;cursor: pointer;"
            >{{dictForm.contentname}}</p>
          </FormItem>
          <FormItem class="lef">
        ...
        <div class="ul" v-for="(item,index) in relativeList">
          <div class="lef dome-relative-list">
            ...
            <Select
              v-model="item.counterpartId"
              style="float: left;text-align: left; width: calc(90% - 108px);"
              @on-change="relativeNameChange(item.counterpartId)"
            >
            ...
            <Icon
              ...
              size="30"
              color="#2d8cf0"
              style="float: left;width: calc(10% - 12px);cursor: pointer;"
              @click="unfoundCounterpart(index)"
            />
          </div>
          <Form-item label="合同地位" class="lef">
            ...
            <Icon
              v-show="!item.removeExit"
              type="md-add-circle"
              size="30"
              color="#2d8cf0"
              style="float: left;width: 10%;cursor: pointer;"
              @click="relativeNameAdd"
            />
            <Icon
              v-show="item.removeExit"
              ...
              color="#2d8cf0"
              style="float: left;width: 10%;cursor: pointer;"
              @click="relativeNameremove(index)"
            />
          </Form-item>
        </div>
        <div class="ul">
          ...
          <FormItem class="lef" v-show="dictForm.istopay != 2">
            <span style="position: absolute;left: -67px;top: 0;">
              <b style="color: #ed4014;font-size: 14px;line-height: 33px;">*</b>合同金额
            </span>
            <InputNumber
              ...
              :max="1000000000"
              :min="0"
              @on-change="nuMinput(dictForm.money)"
              v-model="dictForm.money"
            ></InputNumber>
        ...
        <div class="ul">
          ...
          <FormItem label="合同份数" class="lef">
            <InputNumber
              ...
              :max="1000000000"
              :min="1"
              @on-change="nuMinput2(dictForm.totalnum,3)"
              v-model="dictForm.totalnum"
            ></InputNumber>
          </FormItem>
          <FormItem label="关联主合同" class="lef">
            <Button
              ...
              @click="relevance"
            >{{dictForm.parentContractName?dictForm.parentContractName:"选择主合同"}}</Button>
          </FormItem>
        </div>
        <div class="ul">
        ...
        <div class="ul">
          <FormItem class="lef">
            ...
            <div slot="label">
              <span>
                <i style="color: red;margin-right: 5px;font-size: 14px;">*</i>是否重大合同
              </span>
        ...
        <div class="ul">
          <FormItem label="备注">
            <Input type="textarea" v-model="dictForm.description" :maxlength="1000" :rows="2" />
          </FormItem>
        </div>
        <Col :span="24">
          <FormItem label="相关附件">
      ...
      <div slot="footer">
        ...
        <Button
          v-if="dictForm.modalSource != 1"
          type="primary"
          :loading="btnLoading2"
          @click="handelSubmitDict(2)"
        >
    ...
    <relative-edit
      :formData.sync="relativeDictForm"
      :visible.sync="relativeModalVisible"
      :sourceType.sync="source"
    ...
    <Modal title="范本选择" v-model="modalVisible5" :mask-closable="false" :width="1000" footer-hide>
      <Form>
        <Form-item label="范本名称">
          <Input
            type="text"
            v-model="searchForm1.modelName"
            placeholder="请输入"
            clearable
            style="width: 300px"
          />
          <Button
            @click="handleSearch1(1)"
            type="primary"
            icon="ios-search"
      ...
      <Row>
        <Table
          ...
          @on-selection-change="showSelect1"
          ref="table"
        ></Table>
      </Row>
      <Row type="flex" justify="end" class="page">
        <Page
          :current="searchForm1.pageNumber"
          :total="total1"
    ...
    <Modal title="主合同选择" v-model="modalVisible6" :mask-closable="false" :width="1000" footer-hide>
      ...
      <Row type="flex" justify="end" class="page">
        <Page
          :current="signedForm.pageNum"
          :total="signedTotal"
          :page-size="signedForm.pageSize"
          @on-change="changeSignedPage"
          size="small"
          show-total
          show-elevator
          show-sizer
  ...
  methods: {
    ...
    handleSuccess2(res, file) {
      if (res.result == "success") {
        ...
        this.dictForm.name = res.fileName.substring(
          0,
    ...
    handelSubmitDict(i) {
      ...
      if (this.dictForm.money) {
        ...
        if (
          String(this.dictForm.money).length - 1 ==
          String(this.dictForm.money).indexOf(".")
        ) {
      ...
      fileUpUrlAudit({
        file: "",
        generalNo: this.dictForm.generalNo,
        url: this.dictForm.contracturl
      }).then(res => {
        if (res.result == "success") {
      ...
      if (this.dictForm.contracturl) {
        fileUpUrlAudit({
          ...
          if (res.result == "success") {
            ...
            this.dictForm.generalNo = res.generalNo;
            var fileTag = md5(
              fTag.substring(fTag.lastIndexOf("/") + 1, fTag.lastIndexOf("."))
            ); //此处需要进行MD5加密
            var finalFileUrl =
              fTag.substring(0, fTag.lastIndexOf("/") + 1) +
              fileTag +
              fTag.substring(fTag.lastIndexOf("."));
            this.dictForm.contracturl = res.url;
            var fromUrl = res.url;
            this.myOpenUrl =
              wordEdit +
              "/editURL.html?fromUrl=" +
              fromUrl +
              "&fname=" +
              fname +
              "&fileTag=" +
    ...
    formData33: {
      handler(newValue, oldValue) {
        ...
        if (newValue) {
          ...
          // 获取相对方数据
          eachOther().then(res => {
            this.counterpartArr = res.result.records;
          });
        }
      },
      deep: true
    ...
    formData1: {
      handler(newValue, oldValue) {
        ...
        if (this.dictForm.attachmentname) {
          this.fileDownUrl2 =
            "/zhfw/contract/draft/download?id=" +
            this.dictForm.id +
            "&tag=2&access_token=" +
            this.getStore("accessToken");
        } else {
        ...
        this.dictForm.generalNo = this.dictForm.generalNo
          ? this.dictForm.generalNo
          : "";
