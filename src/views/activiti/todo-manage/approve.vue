          ...
          <Row>
            <Col :span="18" class="screen">
              <Form ref="dictForm" :model="dictForm" :label-width="120">
                ...
                <div class="ul" v-show="numberShow">
                  ...
                  <FormItem label="合同编号" class="lef">
                    <input
                      v-show="numberShow2"
                      type="text"
                      v-model="dictForm.number"
                      @blur.prevent="numberInp(dictForm.number)"
                    />
                ...
                <div
                  class="ul"
                  v-for="(item,index) in dictForm.counterpartList"
                  style="color: #515a6e;font-size: 14px;"
                >
                  <div class="lef">
                    <label
                      for
                      style="text-align: right;width: 140px;display: inline-block;padding: 10px 12px 10px 0;"
                    >{{"相对方"+(index + 1)}}</label>
                ...
                <div class="ul">
                  <FormItem label="相关附件">
                    <p
                      v-for="item in dictForm.uploadList"
                      class="auditLef"
            ...
            <Col :span="6" class="opinion">
              <div v-show="opinionShow">
                <p style="font-weight: 600;height: 60px;line-height: 60px;font-size: 12px;">
                ...
                <Form>
                  <FormItem>
                    <Upload
                      action="/zhfw/system/upload/file"
                      :headers="accessToken"
                    ...
                    >
                      <p id="fileName" class="upload">
                        <Icon type="ios-cloud-upload-outline" size="20" />点击上传
                      </p>
                    </Upload>
                    <p style="color: #d3d3d3;">支持扩展名：doc,.docx文件大小：<=10MB</p>
                    <p
                      @click="fileDowm(opinion.fileName,opinion.fileAddress)"
                ...
                <div v-show="assigneeListShow">
                  <p style="font-weight: 600;height: 60px;line-height: 60px;font-size: 12px;">
                    <span style="color: red;">*</span>下一审批人
                  </p>
                  <Tag
                    v-for="item in applyCount"
                    closable
                    @on-close="applyClose"
                    :key="item.id"
                  >{{ item.nickName }}</Tag>
              ...
              <div class="history" style="padding-bottom: 20px;">
                <div class="li" v-for="(item,i) in historyList" :key="i">
                  ...
                  <p v-show="item.fileName">
                    相关附件:
                    <span
                      @click="fileDowm(item.fileName,item.fileAddress)"
...
export default {
  ...
  methods: {
    ...
    reject2() {
      ...
      this.opinion2.comment = this.opinion.comment;
      rejectExamine2({
        taskId: this.opinion2.id,
        destTaskkey: "contractStart",
    ...
    seeProcess() {
      this.modalFlow = true;
      this.imgUrl = `${flowExample}${
        this.opinion2.procInstId
