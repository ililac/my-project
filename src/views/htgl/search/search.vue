					...
					<Row>
						<Form ref="searchForm" :model="searchForm" inline :label-width="120" class="search-form">
							...
							<Form-item label="合同类型">
								<contract-type v-on:typeListen="typeChange" :typeDate.sync="formData1" :widthDate.sync="myStyle"></contract-type>
							</Form-item>
							<Form-item label="收付款方式">
								 <Checkbox
										:indeterminate="payIndeterminate"
										:value="payCheckAll"
										@click.prevent.native="payAllSelect" style="float: left;">全选</Checkbox>
								<CheckboxGroup v-model="payStateList" @on-change="payAllSelect2"  style="float: left;">
...
<script>
    ...
    export default {
        ...
        methods: {
			...
			//表格导出
			handleExport(){
				window.open("/zhfw/contract/query/exportXls?pageSize=10&pageNum=1&stateIds="+this.searchForm.stateIds+"&name="+this.searchForm.name+"&number="+this.searchForm.number+"&typeId="+this.searchForm.typeId+"&isTopay="+this.searchForm.isTopay+"&startTime="+this.searchForm.startTime+"&endTime="+this.searchForm.endTime+"&draftsManName="+this.searchForm.draftsManName+"&accessToken="+this.accessToken.accessToken);
			},
			allSelect(){
