<template>
	<div>
			...
			<Form ref="dictForm" :model="dictForm" :label-width="120" :rules="dictFormValidate">
				<div class="ul">
					<FormItem label="相对方名称:" class="lef" prop="counterpartName" v-show="!dictForm.disable">
						...
						<Button :loading="btnLoading2" style="float: left;width: 20%;" @click="findCompany" icon="md-refresh">
					...
					<FormItem label="相对方名称:" class="lef" prop="counterpartName" v-show="dictForm.disable">
						...
						<Button disabled :loading="btnLoading2" style="float: left;width: 20%;" @click="findCompany" icon="md-refresh">
							<span v-if="!btnLoading2">同步</span>
							<span v-else>同步</span>
						</Button>
					</FormItem>
				...
				<div class="ul">
					...
					<FormItem label="资质证书:" class="lef">
						<Upload 
							...
							class="upload"
							>
							<p class="upload"><Icon type="ios-cloud-upload-outline" size="20"/><span id="qualificationInfoName">上传文件</span></p>
						</Upload>
				...
				<div class="clear" v-show="detailList == 1">
					...
					<div class="clear" style="height: 40px;line-height: 40px;text-align: center;color: #2d8cf0;cursor: pointer;" @click="linkAdd">+添加联系人</div>
					<Row type="flex" justify="end" class="page">
						<Page :total="dictForm.linkTotal" show-total  :page-size="10" @on-change="pageChange"/>
					</Row>
				</div>
				<div class="clear" v-show="detailList == 2">
					<Table border :columns="stockholders" :data="dictForm.stockholdersList" sortable="custom2" ref="table2"></Table>
					<Row type="flex" justify="end" class="page">
						<Page :total="dictForm.stockholdersTotal" show-total  :page-size="10" @on-change="pageChange"/>
					</Row>
				</div>
				<div class="clear" v-show="detailList == 3">
					<Table border :columns="administrative" :data="dictForm.punishmentInfos" sortable="custom3" ref="table3"></Table>
					<Row type="flex" justify="end" class="page">
						<Page :total="dictForm.administrativeTotal" show-total :page-size="10" @on-change="pageChange"/>
					</Row>
				</div>
				<div class="clear" v-show="detailList == 4">
					<Table border :columns="notice" :data="dictForm.ktannouncements" sortable="custom4" ref="table4"></Table>
					<Row type="flex" justify="end" class="page">
		...
		<Modal title="联系人添加" v-model="linkFormValidate" :mask-closable='false' :width="1000" :styles="{top: '30px'}" class="formModel">
	...
	export default {
		...
		data(){
			return{
				...
				dictFormValidate: {
				    // 表单验证规则
				    counterpartTypeId: [{ required: true, message: "不能为空", trigger: "blur" }],
		...
		methods: {
				...
				if(!this.dictForm.counterpartName){
					...
					animateWidths({"counterpartName":this.dictForm.counterpartName,"id":this.dictForm.counterpartId}).then(res => {
						if(res == "success"){
							...
							dictForm.stockholdersList = JSON.stringify(dictForm.stockholdersList);
			...
			findCompany(){
				...
				relativeFindCompany(this.findCompanyDate).then(res => {
			        ...
			        this.dictForm.punishmentInfos = res.punishmentInfos.punishmentInfos?res.punishmentInfos.punishmentInfos:[];
			        this.administrativeTotal = res.punishmentInfos.total?res.punishmentInfos.total:0;
					this.dictForm.ktannouncements = res.ktannouncements.ktannouncements?res.ktannouncements.ktannouncements:[];
			        this.noticeLisTotal = res.ktannouncements.total?res.ktannouncements.total:0;
					if(res.enterpriseId){
			...
			//资质证书上传之前
			handleBeforeUpload(){
				document.getElementById("qualificationInfoName").innerHTML = "文件上传中...";
			},
			//合同正文中上传文件的格式错误
